...
class LocalNetworkConfigPlugin(base.BasePlugin):
    ...
    def _configure_interfaces_dhcp(self, config: list, links: list):
        ...
        network_data_version = config.get("version")
        if config and config.get('config'):
            config = config.get('config')

        # Check if v2.
        if network_data_version == 2:
            # Loop through interface types to configure.
            for item_type in [
                    network_utils.NETWORK_LINK_TYPE_ETHERNET,
                    network_utils.NETWORK_LINK_TYPE_BOND,
                    network_utils.NETWORK_LINK_TYPE_VLAN]:
                config_items = config.get(item_type + 's', {})

                # Loop through any configurations for this type.
                for name, config_item in config_items.items():
                    ...
                    for link in links:
                        ...
                        linkInConfig = False
                        bonds = config.get(
                            network_utils.NETWORK_LINK_TYPE_BOND + 's', {})
                        if link.id in bonds:
                            linkInConfig = True
                            break
        ...
        for config_item in config:
            ...
            if config_item.get("type") not in [
            ...
            for subnet in subnets:
                ...
                if subnet_type == network_utils.SUBNET_TYPE_DHCP:
                    dhcpv4Enabled = True
                elif subnet_type == network_utils.SUBNET_TYPE_DHCP4:
                    dhcpv4Enabled = True
    ...
    def execute(self, service, shared_data):
        ...
        if CONF.local_network_config.once:
            status = self.osutils.get_config_value(KEY)
            if status == 1:
        ...
        self.osutils.set_config_value(KEY, 1)
